...
#include <cmath>
//#include <TRandom3.h>
#include "random.h" //* utilizzo la classe Random di Galli
#include "constants.h"
#include "functions.h"
...
{
	return 1.0; // costante
	//return exp(-0.5*v*v); // gaussiana
    //return exp(-0.5*(v-mean)*(v-mean)/(s*s)) + exp(-0.5*(v+mean)*(v+mean)/(s*s)); // doppia gaussiana
}
...
double variationalWaveFunction_second(double v)
{
    ...
    //return exp(-0.5*(v-mean)*(v-mean)/(s*s))*( (v-mean)*(v-mean)/(s*s) - 1 )/(s*s) + exp(-0.5*(v+mean)*(v+mean)/(s*s))*( (v+mean)*(v+mean)/(s*s) - 1 )/(s*s);
}
...
void brownianBridge()
{
	...
	for(int i=0;i<brownianBridgeReconstructions;i++)
	{
		...
		double variance = 2*lambda*dtau*left_reco/(left_reco+1); //* perchè left_reco/(left_reco+1)?
...
void consoleOutput()
{
	...
	if(PIGS)
		cout<<"BM: "<<(100*(double)acceptedBM)/totalBM << " % " << endl;
	cout<<"Transl: "<<(100*(double)acceptedTranslations)/totalTranslations << " % "<< endl;
	cout<<"BB: "<<(100*(double)acceptedBB)/totalBB << " % "<< endl;
}
...
{
	ofstream out("Temp/PIGS/potential_const.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("Temp/PIGS/kinetic_const.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("Temp/PIGS/probability_const.dat");
