...
void Equilibrate(){
    ...
    while(a == 'y'){
        cout << "Choose the number of steps to equilibrate: " << endl;
        cin >> n_eq;
        cout << endl;
...
//Pressure
    Pres << setw(wd) << glob_av[iw]/(double)iblk << setw(wd) << err_press << endl;
...
//g(r)
    ...
    for(int i=0; i < nbins; i++){
        ...
        gdir = blk_av[igofr + i]/blk_norm/(double)npart/rho/delta_V; //g(r)
...
void SetConfig(){
    ...
    cout << "Do you want to start from a previous configuration? [y/n]. If n the simulation will start from an fcc configuration." << endl;
    
    cin >> mode;
    
    while(mode!="y" && mode!="n"){
        
        cout << "Invalid input. Choose 'y' (previous configuration) or 'n' (fcc configuration)." << endl;
        cin >> mode;
    }
...
void Print() //Print instant values of potential energy per particle and pressure
{
    ...
    //Print instant potential eneergy per particle
    Epot << epot_inst << endl;
    //Pressure
    Pres << pres_inst << endl;
...
void PrintVideo(void) //Print instant values on video (after equilibration)
{
    
    cout << "Potential energy per particle: " << walker[iv]/(double)npart + vtail << endl;//Instant potential energy per particle
    cout << "Pressure: " << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl << endl;//Instant Pressure
